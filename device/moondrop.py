...
class Moondrop:
    ...
    def _get_backend() -> Any:
        ...
        if os.name != "nt" or libusb1 is None:
            return None
        ...
        try:
            ...
            logging.warning(
                "libusb-package is not installed; falling back to PyUSB default backend."
            )
            return None
    ...
    def _get_candidate_ids(config: AppConfig) -> List[Tuple[str, int, int]]:
        """Return all known original-Dawn-Pro-compatible USB IDs."""
        candidates = [
            (
                "Moondrop Dawn Pro",
        ...
        for item in config.device_identifiers.ADDITIONAL_DEVICE_IDS:
            try:
                ...
                logging.warning("Skipping invalid additional device identifier: %s", item)
    ...
    def __init__(self, config: AppConfig) -> None:
        ...
        if usb is None:
            raise ValueError("PyUSB is not installed. Install pyusb to use original Dawn Pro USB controls.")
        ...
        if self.device is None:
            ...
            raise ValueError(f"Original Dawn Pro USB device not found. Supported IDs: {supported_ids}.")
        logging.info("Device found and initialized: %s", self.device_name)
